<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8" %>
  <!DOCTYPE html>
  <html lang="en">

  <head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="../assets/css/daviplata_login.css">
    <title>Login_dp</title>
  </head>

  <body>
<!-- Contenidos de el header -->
    <div class="contenedor">
      <div class="header">
        <!-- Servlet para una imagen que redirige a la vita principal -->
        <div id="img"><a href="bolsillo?accion=index">
            <img
              src='data:image/png;base64,' />
          </a>
        </div>
        <h3><a href="bolsillo?accion=index">HOME</a> </h3>
        <h1>DAVIPLATA</h1>
      </div>

      <div class="centro">
        <!--contednidos de el centro -->
        <h3>INICIA SESION </h3>
        <!-- fondo de el formulario -->
        <div id='daviplata_login' class='daviplata_login'>
          <div id='rectangle8' class='rectangle8'></div>


          <div class="formulario_login">
            <!-- Formulario de inicio de sesion tipo post ( "envio ") -->
            <form action="bolsillo" method="post">
              <label for="nombre">Nombre:</label>
              <!--Input de tipo texto cuyo nombre sera "nombre" 
              la informacion ingresada en este input sera invocada por el nombre 
            "nombre "-->
              <input type="text" id="nombre" name="nombre" required>
              <label for="documento">Numero de documento:</label>
              <!--Input de tipo texto cuyo nombre sera "documento" 
              la informacion ingresada en este input sera invocada por el nombre 
            "documento"-->
              <input type="text" id="documento" name="documento" required> 
             
              <label for="contrasena">Contraseña:</label>
               <!--Input de tipo texto cuyo nombre sera "contraseña" 
              la informacion ingresada en este input sera invocada por el nombre 
            "contraseña"-->
              <input type="password" id="contrasena" name="contrasena" required> 
              <!-- Boton  (llamado accion cuyo valor sera inicio) de finalizacion y envio de el formulario 
                a ala clase del servlet donde enviarelos la informacion 
              a un metodo de validacion -->
              <button type="submit" name="accion" value="inicio">Ingresar</button>
            </form>
          </div>
        </div>


      </div>
    </div>

  </body>

  </html>